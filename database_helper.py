...
class DatabaseHelper:
    ...
    def assign_ticket(self, ticket_id, assignee):
        ...
        try:
            ...
            cursor.execute("""
                UPDATE tickets 
                SET assigned_to = %s, updated_at = GETDATE()
                WHERE id = %s
            """, (assignee, ticket_id))
    ...
    def add_ticket_comment(self, ticket_id, comment, admin_name):
        ...
        try:
            ...
            cursor.execute("""
                INSERT INTO ticket_comments (ticket_id, comment, admin_name)
                VALUES (%s, %s, %s)
    ...
    def create_timeoff_request(self, employee_name, start_date, end_date, reason):
        ...
        try:
            ...
            cursor.execute("""
                INSERT INTO timeoff_requests (employee_name, start_date, end_date, reason)
                VALUES (%s, %s, %s, %s)
    ...
    def update_timeoff_status(self, request_id, status):
        ...
        try:
            ...
            print(f"Error updating timeoff status: {e}")
            return False
    ...
    def create_leave_request(self, employee_id, employee_name, leave_type, start_date, end_date, reason):
        ...
        try:
            ...
            # For now, use a default employee_id since we don't have proper user ID mapping
            # This is a temporary fix until we implement proper user ID lookup
            default_employee_id = 1
            
            cursor.execute("""
                INSERT INTO leave_requests (employee_id, employee_name, leave_type, start_date, end_date, reason)
                VALUES (%s, %s, %s, %s, %s, %s)
            """, (default_employee_id, employee_name, leave_type, start_date, end_date, reason))
    ...
    def get_leave_requests_by_employee(self, employee_id):
        ...
        try:
            ...
            # If employee_id is an email, we need to handle it differently
            if isinstance(employee_id, str) and '@' in employee_id:
                # For now, return empty list since we don't have proper user ID mapping
                # This is a temporary fix until we implement proper user ID lookup
                cursor.close()
                return []
        ...
        try:
            ...
            if approved_by:
                cursor.execute("""
                    ...
                    SET status = %s, approved_by = %s, approved_at = GETDATE()
                    WHERE id = %s
    ...
    def add_feedback_comment(self, feedback_id, comment, admin_name):
        ...
        try:
            ...
            cursor.execute("""
                INSERT INTO feedback_comments (feedback_id, comment, admin_name)
                VALUES (%s, %s, %s)
        ...
        try:
            ...
            if reviewed_by:
                cursor.execute("""
                    ...
                    SET status = %s, reviewed_by = %s, reviewed_at = GETDATE()
                    WHERE id = %s
    ...
    def create_notification(self, user_email, message, notification_type='info'):
        ...
        try:
            ...
            cursor.execute("""
                ...
                VALUES (%s, %s, %s, %s)
            """, (user_email, "System Notification", message, notification_type))
    ...
    def create_service_request(self, user_email, request_type, subject, description, priority='medium'):
        ...
        try:
            ...
            cursor.execute("""
                INSERT INTO service_requests (user_email, request_type, subject, description, priority)
                VALUES (%s, %s, %s, %s, %s)
        ...
        try:
            ...
            if assigned_to:
                cursor.execute("""
                    ...
                    SET status = %s, assigned_to = %s, updated_at = GETDATE()
                    WHERE id = %s
    ...
    def assign_service_request(self, request_id, assigned_to):
        ...
        try:
            ...
            cursor.execute("""
                UPDATE service_requests 
                SET assigned_to = %s, status = 'assigned', updated_at = GETDATE()
                WHERE id = %s
    ...
    def add_service_request_comment(self, request_id, comment, admin_name):
        ...
        try:
            ...
            cursor.execute("""
                INSERT INTO service_request_comments (request_id, comment, admin_name)
                VALUES (%s, %s, %s)
    ...
    def create_internal_job(self, title, department, location, job_type, description, requirements, salary_range, posted_date, deadline_date):
        ...
        try:
            ...
            cursor.execute("""
                INSERT INTO internal_jobs (title, department, location, job_type, description, requirements, salary_range, posted_date, deadline_date)
    ...
    def get_internal_job_by_id(self, job_id):
        ...
        try:
            ...
            if job_data:
                return dict(zip(columns, job_data))
            return None
        except Exception as e:
    ...
    def apply_for_internal_job(self, job_id, user_email, resume_path=None, cover_letter=None):
        ...
        try:
            ...
            cursor.execute("""
                INSERT INTO internal_job_applications (job_id, user_email, resume_path, cover_letter)
                VALUES (%s, %s, %s, %s)
    ...
    def create_timesheet(self, user_id, date, hours_worked, description):
        """Create a timesheet entry"""
        try:
            connection = self.get_connection()
            cursor = connection.cursor()
            
            # For now, use a default user_id since we don't have proper user ID mapping
            # This is a temporary fix until we implement proper user ID lookup
            default_user_id = 1
            
            cursor.execute("""
                INSERT INTO timesheets (user_id, date, hours_worked, description)
                VALUES (%s, %s, %s, %s)
            """, (default_user_id, date, hours_worked, description))
    ...
    def get_timesheets_by_employee(self, user_email):
        """Get timesheets by employee email"""
        try:
            connection = self.get_connection()
            cursor = connection.cursor()
            ...
            # Original code commented out due to user_id column issue
            # Get user_id from email
            # user = self.get_user_by_email(user_email)
            # if not user:
            #     return []
            # 
            # cursor.execute("SELECT * FROM timesheets WHERE user_id = %s ORDER BY date DESC", (user['id'],))
            # columns = [column[0] for column in cursor.description]
            # timesheets = []
            # for row in cursor.fetchall():
            #     timesheets.append(dict(zip(columns, row)))
            # cursor.close()
            # return timesheets
        except Exception as e:
    ...
    def get_all_timesheets(self):
        ...
        try:
            ...
            cursor.execute("SELECT * FROM timesheets ORDER BY created_at DESC")
            columns = [column[0] for column in cursor.description]
            timesheets = []
            for row in cursor.fetchall():
                timesheets.append(dict(zip(columns, row)))
    ...
    def add_timesheet_comment(self, timesheet_id, comment, admin_name):
        ...
        try:
            ...
            cursor.execute("""
                INSERT INTO timesheet_comments (timesheet_id, comment, admin_name)
                VALUES (%s, %s, %s)
    ...
    def create_skills_assessment(self, user_email, test_type, score, total_questions, answers, completed_at):
        """Create a skills assessment record"""
        try:
            connection = self.get_connection()
            cursor = connection.cursor()
    ...
    def get_latest_skills_assessment(self, user_email):
        ...
        try:
            ...
            cursor.execute("""
                SELECT TOP 1 * FROM skills_assessments 
                WHERE user_email = %s 
            ...
            if assessment_data:
                return dict(zip(columns, assessment_data))
            return None
        except Exception as e:
    ...
    def get_user_learning_roadmap(self, user_email):
        ...
        try:
            ...
            cursor.execute("""
                SELECT TOP 1 lr.*, sa.score, sa.test_type
            ...
            if roadmap_data:
                return dict(zip(columns, roadmap_data))
            return None
        except Exception as e:
    ...
    def get_course_by_id(self, course_id):
        ...
        try:
            ...
            if course_data:
                return dict(zip(columns, course_data))
            return None
        except Exception as e:
