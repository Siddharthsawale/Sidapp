...
def login():
    if request.method == "POST":
        ...
        try:
            # Get user from database (password check removed per user request)
            user = db_helper.get_user_by_email(email)
            
            if user:
                # Set session without password verification
                session["user"] = email
                ...
                flash("User not found. Please contact your administrator.", "error")
        except Exception as e:
...
def admin_dashboard():
    ...
    # Get all data from database with error handling
    try:
    ...
    def safe_count_status(items, status_key, target_status):
        try:
            return len([item for item in items if item.get(status_key) == target_status])
        except:
            return 0
    
    admin_data = {
        "total_tickets": len(db_tickets),
        "open_tickets": safe_count_status(db_tickets, "status", "open"),
        "resolved_tickets": safe_count_status(db_tickets, "status", "resolved"),
        "total_requests": len(db_timeoff) + len(db_leave_requests),
        "pending_requests": safe_count_status(db_timeoff, "status", "pending") + safe_count_status(db_leave_requests, "status", "pending"),
        "total_timesheets": len(db_timesheets),
        "pending_timesheets": safe_count_status(db_timesheets, "status", "pending"),
        "total_feedback": len(db_feedback),
        "pending_feedback": safe_count_status(db_feedback, "status", "pending"),
        "total_applications": len(db_applications),
        "pending_applications": safe_count_status(db_applications, "status", "pending"),
